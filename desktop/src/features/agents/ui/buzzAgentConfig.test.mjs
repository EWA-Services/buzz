...
// ---------------------------------------------------------------------------
// HarnessNativeEffortFields mutation contract
// ---------------------------------------------------------------------------
//
// Tests for the save/clear/unrelated-edit behavior of HarnessNativeEffortFields.
// The mutation logic mirrors handleChange in buzzAgentModelTuningFields.tsx:
//   save: native key written, legacy key atomically deleted
//   clear: both native and legacy keys deleted
//   unrelated edit: other env keys preserved
//
// These are pure-logic tests (no React rendering required) because the contract
// is deterministic given input state and selected value.
//
...
/**
 * Applies the HarnessNativeEffortFields mutation contract for a given value.
 * This IS the specification: native write + legacy delete on save; delete both on clear.
 * Tests below verify the contract holds by applying it to representative inputs.
 */
function applyHarnessNativeChange(envVars, nativeKey, value) {
  const next = { ...envVars };
  if (value === "") {
  ...
  // Always delete the legacy key — prevents stale alias resurrection.
...
test("harness_native_save_writes_native_key_and_deletes_legacy", () => {
  ...
  const initial = { [BUZZ_AGENT_THINKING_EFFORT]: "high" };
  const result = applyHarnessNativeChange(initial, GOOSE_NATIVE_KEY, "medium");
...
test("harness_native_clear_deletes_both_native_and_legacy_keys", () => {
  ...
  const result = applyHarnessNativeChange(initial, GOOSE_NATIVE_KEY, "");
  assert.equal(
...
test("harness_native_save_does_not_disturb_unrelated_env_vars", () => {
  ...
  const result = applyHarnessNativeChange(initial, GOOSE_NATIVE_KEY, "off");
