...
//
// Simulates up to three synthetic agents "working" / "making progress" so the
...
export type DebugAgentSlotId = "a" | "b" | "c";

export const DEBUG_AGENT_SLOTS: readonly DebugAgentSlotId[] = ["a", "b", "c"];

/** Fixed synthetic identities (valid lowercase 64-hex pubkeys). Names are
 * chosen so avatar initials differ per slot (AD / BD / GD — see getInitials). */
...
> = {
  ...
  c: { pubkey: "dc0c".repeat(16), name: "Gamma Debug" },
};
...
function createSlotState(slot: DebugAgentSlotId): DebugAgentSlotState {
  return {
 ...
 * transcript parser renders (see agentSessionTranscript.ts) and the tool
 * classifier labels distinctly (see agentSessionToolClassifier.ts): thoughts,
 * a plan checklist, a file read, shell commands (one failing), a file edit,
 * a Buzz relay search, a streamed three-chunk assistant message, a todo
 * update, and a usage tick. Frames are emitted as `acp_read` events — that
 * is the kind the transcript parser expects for `session/update`.
 */
...
> = [
  ...
  ({ cycle }) => ({
    sessionUpdate: "plan",
    entries: [
      { status: "completed", content: "Trace the live-activity feed" },
      {
        status: cycle % 2 === 1 ? "completed" : "in_progress",
        content: "Verify pill labels per action kind",
      },
      { status: "pending", content: "Capture the screenshot matrix" },
    ],
  }),
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call",
    toolCallId: `debug-read-${slot}-${cycle}`,
    toolName: "read_file",
  ...
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call",
    toolCallId: `debug-grep-${slot}-${cycle}`,
    toolName: "shell",
    title: "Find pill call sites",
    kind: "execute",
    status: "in_progress",
    rawInput: { command: 'rg -n "ActivityPill" desktop/src' },
  }),
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call_update",
    toolCallId: `debug-grep-${slot}-${cycle}`,
    status: "completed",
    rawOutput: { summary: "12 matches across 4 files" },
  }),
  () => ({
    sessionUpdate: "agent_thought_chunk",
    content: {
      type: "text",
      text: "The ticker keys off transcript item ids — editing the row component next.\n",
    },
  }),
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call",
    toolCallId: `debug-edit-${slot}-${cycle}`,
    toolName: "str_replace",
    title: "Edit composer activity row",
    kind: "edit",
    status: "in_progress",
    rawInput: {
      path: "desktop/src/features/channels/ui/ChannelComposerActivityRow.tsx",
    },
  }),
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call_update",
    toolCallId: `debug-edit-${slot}-${cycle}`,
    status: "completed",
    rawOutput: { summary: "Replaced 1 occurrence" },
  }),
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call",
    toolCallId: `debug-search-${slot}-${cycle}`,
    toolName: "search",
    title: "Searching relay history",
    kind: "search",
    status: "in_progress",
    rawInput: { query: "composer live activity" },
  }),
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call_update",
    toolCallId: `debug-search-${slot}-${cycle}`,
    status: "completed",
    rawOutput: { summary: "3 threads matched" },
  }),
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call",
    toolCallId: `debug-test-${slot}-${cycle}`,
    toolName: "shell",
    title: "Run composer unit tests",
    kind: "execute",
    status: "in_progress",
    rawInput: { command: "pnpm vitest run composerLiveActivity" },
  }),
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call_update",
    toolCallId: `debug-test-${slot}-${cycle}`,
    status: "failed",
    rawOutput: {
      error: "2 assertions failed in composerLiveActivity.test.mjs",
    },
  }),
  ({ slot, cycle }) => ({
  ...
  ({ slot, cycle }) => ({
    sessionUpdate: "tool_call",
    toolCallId: `debug-todo-${slot}-${cycle}`,
    toolName: "todo",
    title: "Update todos",
    kind: "think",
    status: "completed",
    rawInput: {
      todos: [
        { text: "Fix failing pill-label assertions", checked: false },
        { text: "Re-run the screenshot matrix", checked: false },
      ],
    },
  }),
  ({ cycle }) => ({
...
function randomProgressDelayMs(): number {
  return (
    PROGRESS_DELAY_MIN_MS +
    Math.random() * (PROGRESS_DELAY_MAX_MS - PROGRESS_DELAY_MIN_MS)
  );
}

/**
 * Self-rescheduling frame loop: each frame picks a fresh random delay so the
 * stream feels organic instead of metronomic. Cancelled via stopProgressTimer
 * (both progress-off and working-off paths run it).
 */
function scheduleNextProgressFrame(slot: DebugAgentSlotId) {
  const runtime = runtimes[slot];
  runtime.progressTimer = setTimeout(() => {
    emitNextProgressFrame(slot);
    scheduleNextProgressFrame(slot);
  }, randomProgressDelayMs());
}
