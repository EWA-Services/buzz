...
///   scheme checked on the parsed (normalized) value, and whose UTF-8 byte
///   length is ≤ 2 048.  Both Rust's `url` crate and the browser's `new URL()`
///   implement the WHATWG URL Standard, so parse-first makes both sides run the
///   same algorithm — including shorthand forms like `http:example.com` which
///   both normalize to `http://example.com/`.
...
pub fn is_safe_catalog_avatar_url(url: &str) -> bool {
    ...
    // Candidate HTTP/HTTPS URLs: byte cap → whitespace/paren guard → WHATWG parse → scheme check.
    //
    // We parse all candidates rather than requiring a literal `http://`/`https://`
    // prefix, because the WHATWG URL Standard normalizes shorthand forms like
    // `http:example.com` to `http://example.com/` — both Rust and TypeScript
    // accept them — and a literal prefix gate would reject them before parsing.
    // The 2 048-byte cap acts as the performance gate instead.
    //
    // Only check this path for strings that could plausibly be HTTP/HTTPS URLs
    // (i.e. not data: URIs which are handled below).
    if !url.starts_with("data:") {
        // UTF-8 byte length cap — same unit as TS `byteLength`.
        if url.len() > MAX_HTTP_URL_BYTES {
            return false;
        }
        // Reject URLs containing ECMAScript-`\s` whitespace or parentheses,
        // matching TypeScript's pre-check `/[\s()]/u.test(value)`.
        //
        // Exact ECMAScript `\s` equivalence in Rust:
        //   ECMAScript `\s` = char::is_whitespace() − U+0085 (NEL) + U+FEFF (BOM)
        //   Proof: `/\s/u.test('\u0085')` → false (JS excludes NEL);
        //          `'\u{FEFF}'.is_whitespace()` → false (Rust excludes BOM).
        //   url::Url::parse percent-encodes these characters rather than rejecting
        //   them, so without the guard the two validators would diverge.
        if url.chars().any(|c| {
            ((c.is_whitespace() && c != '\u{0085}') || c == '\u{FEFF}') || c == '(' || c == ')'
        }) {
            return false;
        }
        // Parse with the same WHATWG algorithm as TypeScript's `new URL()`.
        // url::Url::parse rejects malformed authority components (https://^,
        // https://a:b) and normalizes the scheme so HTTPS://example.com and
        // http:example.com are handled identically on both sides.
