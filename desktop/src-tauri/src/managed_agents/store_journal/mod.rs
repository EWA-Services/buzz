...
pub use events::{insert_inbox_event, insert_outbox_event, mark_outbox_published};
#[cfg(test)]
pub use events::{read_inbox_events, read_outbox_events, InsertEventOutcome};
...
pub use txn::{mutate_store, run_boot_recovery, StoreState};
#[cfg(test)]
...
/// Advance a journal operation from `Pending` to `Committed` after a successful
/// file write. Best-effort: a journal-open or CAS failure is logged and ignored
/// so a journal hiccup never prevents the write from being acknowledged.
pub fn advance_to_committed(app: &tauri::AppHandle, op_id: &str) {
    if let Ok(anchor) = anchor::store_anchor_dir(app) {
        if let Ok(journal) = schema::open_journal(&anchor) {
            let _ = operations::advance_disposition(
                &journal,
                op_id,
                &operations::Disposition::Pending,
                &operations::Disposition::Committed,
            );
        }
    }
}
