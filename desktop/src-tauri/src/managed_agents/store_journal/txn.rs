    ...
    F: FnOnce(StoreState<'_>) -> Result<(Vec<ManagedAgentRecord>, Vec<TeamRecord>, T), String>,
{
    ...
    let agents_stage = agents_path.with_extension("json.stage");
    let teams_stage = teams_path.with_extension("json.stage");
        ...
        match phase {
            FileCommitPhase::Intent => {
                ...
                if agents_stage.exists() {
                    ...
                    if let Err(e) = advance_file_commit_phase(
                        journal,
                        &commit_id,
                ...
                if teams_stage.exists() {
                    ...
                    // Teams stage missing but agents was present: advance to committed
                    // (teams canonical must already be there, or there was nothing to write).
                    advance_file_commit_phase(journal, &commit_id, &FileCommitPhase::Committed)
                        .unwrap_or_else(|e| {
                            eprintln!(
                                "buzz-desktop: boot-recovery: advance committed (no teams stage) failed: {e}"
                            );
                        });
                }
            }
...
///
/// This is the **single publication authority** for B1.  It:
/// 1. Builds the signed Nostr event for `record` (agent or team).
/// 2. Inserts the outbox row (immutable event identity) into the open journal
///    transaction — `?`-propagated so a collision or journal failure aborts
///    the mutation.
/// 3. Inserts the retention row with `pending_sync = true` — `?`-propagated.
/// 4. Returns the event identity `(event_id, raw_json)` so the operation ID
///    can be linked by the caller.
///
