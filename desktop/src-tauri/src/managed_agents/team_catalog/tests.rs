...
fn test_oversized_avatar_on_a_non_builtin_fails_the_size_contract() {
    // Non-built-in oversized avatar produces an error; owner can act on it.
...
run_fixture_table!(
    ...
    ("valid_uppercase_hash", "valid_uppercase_hash.json"),
...
fn test_real_builtin_without_avatar_mutation_projects_successfully() {
    // A real built-in (fizz) has a ~170 KiB avatar that exceeds MAX_AVATAR_URL_BYTES.
    // The oversized avatar is stripped inside member_projection (built-in path only),
    // so publish succeeds and the projected member carries no avatar field.
    let builtin =
