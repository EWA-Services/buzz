...
//   * restart hash  — `spawn_config_hash` hashes the effective value
...
mod tests {
    ...
    // ── Spawn hash: above-cap equivalence + cap crossing ──────────────────────
    //
    // An over-cap edit (10 → 8, both cap to 5) must not raise the restart badge.
    // A cross-cap edit (8 → 3, 8 caps to 5 but 3 is below cap) must raise it.

    fn hash_for(command: &str, parallelism: u32) -> u64 {
        use crate::managed_agents::{spawn_hash::spawn_config_hash, GlobalAgentConfig};
        ...
        spawn_config_hash(&record, &[], &[], "wss://relay.example", &global)
    }

    /// Two over-cap values (10 and 8) hash the same for OpenClaw: both clamp to 5.
    #[test]
    fn hash_above_cap_equivalence() {
        assert_eq!(
            hash_for("openclaw", 10),
            hash_for("openclaw", 8),
            "parallelism 10 and 8 both clamp to 5 for OpenClaw — hash must be equal"
        );
    }

    /// Cap-crossing edit (8 → 3) hashes differently: 8 caps to 5 but 3 does not.
    #[test]
    fn hash_cap_crossing_inequality() {
        assert_ne!(
            hash_for("openclaw", 8),
            hash_for("openclaw", 3),
            "parallelism 8 (clamps to 5) and 3 (no clamp) must hash differently"
        );
    }
