...
///
/// Wrapped in a single transaction so a crash before COMMIT leaves the DB fully
/// pre-migration and the marker absent; the next open re-runs from scratch.
fn migrate_add_cache_read_tokens(conn: &Connection) -> Result<(), String> {
    ...
    // Add turn column when absent.
    let turn_exists: bool = {
        let mut stmt = tx
            .prepare("PRAGMA table_info(agent_metric_index)")
        ...
        names.iter().any(|n| n == "turn_cache_read_tokens")
    };
    if !turn_exists {
        tx.execute_batch("ALTER TABLE agent_metric_index ADD COLUMN turn_cache_read_tokens TEXT")
            .map_err(|e| format!("migration M2: ALTER TABLE (turn) failed: {e}"))?;
        tx.execute_batch(
            "ALTER TABLE agent_metric_index ADD COLUMN cumulative_cache_read_tokens TEXT",
        )
        .map_err(|e| format!("migration M2: ALTER TABLE (cumulative) failed: {e}"))?;
    }
